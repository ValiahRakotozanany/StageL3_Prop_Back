...
public class MenuDynamique extends ClassMAPTable {
    ...
    public static ArrayList<ArrayList<MenuDynamique>> getElementMenu(HttpServletRequest request, historique.MapUtilisateur user, CNAPSUser cnapsUser) {
        ...
        try {         
            ...
            String reketyInterdit = "select MENUDYNAMIQUE.* from MENUDYNAMIQUE inner join usermenu on usermenu.IDMENU=MENUDYNAMIQUE.ID  where ( refuser='" + user.getRefuser() + "' OR idrole='" + user.getIdrole() + "') order by MENUDYNAMIQUE.niveau,MENUDYNAMIQUE.rang asc";
            String reketyAutorise = "select MENUDYNAMIQUE.* from MENUDYNAMIQUE inner join usermenu on ( usermenu.IDMENU=MENUDYNAMIQUE.ID)  where (interdit=0 OR interdit is null) and (  refuser='" + user.getRefuser() + "' OR idrole='" + user.getIdrole() + "') order by MENUDYNAMIQUE.niveau,MENUDYNAMIQUE.rang asc";
            System.out.println("rekety interdit == "+reketyInterdit);
            ...
            for (MenuDynamique elt : tabMenu) {    
                if(listInterdit.contains(elt) || !listAutorise.contains(elt) ){
                    System.out.println(" MENNIUUUUU");
                    continue;
                }
                if (arbre.isEmpty()) {                             
                    arbreNiveau.add(elt);                    
                    arbre.add(arbreNiveau);
                     System.out.println(" MENNIUUUUU elsess 11");
                } else {
                                        System.out.println(" MENNIUUUUU elsess");
...
//                    String redirection = elt.getNiveau() == 2 ? "onclick=\"window.location.href='"+getHref(elt)+"'\"" : ""; 
                    ...
                    /*******************************/
                    ret+="<li><a href='"+getHref(elt)+"' class=' " +classCurrent+ "' "+redirection+" ><i class='fa "+elt.getIcone()+ "'></i> <span>"+libtemp1+"</span> <i class='fa fa-angle-left pull-right'></i></a>\n" +
